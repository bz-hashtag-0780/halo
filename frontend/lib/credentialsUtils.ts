...
// Meeting link credential subject interface (exact schema order, no manual ID)
export interface MeetingLinkCredentialSubject {
	meeting_url: string; // 1. Primary field
	creator_address: string; // 2. Who created it
	created_timestamp: string; // 3. When created
	platform: string; // 4. Required by schema - e.g. Zoom, Google Meet
	trust_level: string; // 5. Required by schema - e.g. verified, organization
	expires_at: string; // 6. Required by schema - credential expiration
}
...
// Generate credential subject for meeting link (exact schema order, ID auto-generated)
export const generateMeetingLinkCredential = (
	...
	// Credential subject in exact schema order - ID will be auto-generated by widget
	return {
		meeting_url: meetingUrl,
		creator_address: creatorAddress,
		created_timestamp: now.toISOString(),
		platform: detectPlatform(meetingUrl), // Required schema field
		trust_level: 'verified', // Required schema field
		expires_at: new Date(
			now.getTime() + 30 * 24 * 60 * 60 * 1000
		).toISOString(), // Required schema field - 30 days
	};
};
...
export const useCredentialIssuance = () => {
	...
	const generateWidget = async (
		...
		try {
			...
			console.log(
				'🐛 Generated credentialSubject in exact schema order (no manual ID):',
				{
					...
					trust_level: credentialSubject.trust_level,
					expires_at: credentialSubject.expires_at,
			...
			widgetRef.current.on('issueCompleted', async () => {
				...
				try {
					...
					if (onPresentationGenerated) {
						...
						// Create a proper credential object with complete structure
						// In a real scenario, we'd get this from the AIR service
						const mockCredential = {
							'@context': [
								'https://www.w3.org/2018/credentials/v1',
							],
							id: `urn:credential:${credentialSubject.meeting_url}`,
							type: [
	...
	const issueCredential = async (
		...
		try {
			...
			if (widgetRef.current) {
				...
				// 🎬 DEMO MODE: Auto-complete credential flow after 5 seconds
				console.log(
					'🎬 DEMO MODE: Auto-completing credential flow in 5 seconds...'
				);
				setTimeout(() => {
					if (onPresentationGenerated) {
						console.log(
							'🎬 DEMO: Simulating successful credential issuance...'
						);

						// Create realistic demo credential result
						const demoCredential = {
							'@context': [
								'https://www.w3.org/2018/credentials/v1',
							],
							id: `urn:credential:demo-${Date.now()}`,
							type: [
								'VerifiableCredential',
								'MeetingLinkCredential',
							],
							issuer: ISSUER_DID,
							issuanceDate: new Date().toISOString(),
							credentialSubject: generateMeetingLinkCredential(
								meetingUrl,
								creatorAddress
							),
						};
						...
						const demoPresentation = {
							...
							verifiableCredential: [demoCredential],
							proof: {
								type: 'EcdsaSecp256k1Signature2019',
								created: new Date().toISOString(),
								proofPurpose: 'authentication',
								verificationMethod: `${creatorAddress}#key-1`,
								jws: `demo-proof-${Date.now()}-${Math.random()
									.toString(36)
									.substring(2, 8)}`,
							},
						};
						...
						const shareableUrl = `${meetingUrl}${separator}halo_proof=${encodedProof}`;

						// Trigger success with complete demo result
						const demoResult = {
							success: true,
							credential:
								demoCredential as unknown as JsonDocumentObject,
							presentation: JSON.stringify(demoPresentation),
							shareableUrl: shareableUrl,
						};

						console.log(
							'🎬 DEMO: Presentation generated successfully:',
							demoResult
						);
